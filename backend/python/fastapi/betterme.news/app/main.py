...
#comment
from database.mongodb.rac_database import creRAC, takeRAC
...
async def _scraping(
  ...
  data = test_create_data(url,name,html_type,thumbnail_link,slide_show_link,description,tags)
...
async def _confirm(
  ...
  processContentThread = Thread(target = new_data(check_list,waiting_dict))
  processContentThread.start()
